...
class Settings {
	...
	public static function get( $var = null ) {
		if ( $var ) {
			...
			if ( isset( $field['value'] ) ) {
				...
				return apply_filters( 'murmurations_node_get_setting', self::$settings[ $var ], $var );
			} else {
				return apply_filters( 'murmurations_node_get_setting', false, $var );
			}
		} else {
			return apply_filters( 'murmurations_node_get_settings', self::$settings );
		}
	}
	...
	public static function set( $var, $value ) {
		$value                  = apply_filters( 'murmurations_node_set_setting', $value, $var );
	...
	public static function load() {
		$settings = get_option( 'murmurations_node_settings' );
		...
		self::$settings = apply_filters( 'murmurations_node_load_settings', $settings );
	}
	...
	public static function save() {
		$settings = apply_filters( 'murmurations_node_save_settings', self::$settings );
		return update_option( 'murmurations_node_settings', $settings );
	}
	...
	private static function get_schema_json() {
		return file_get_contents( MURMNODE_ROOT_PATH . 'admin_fields_jschema.json' );
	}
	...
	public static function load_schema( $default_values = null ) {
		...
		$settings_schema = apply_filters( 'murmurations_node_load_settings_schema', $settings_schema );
